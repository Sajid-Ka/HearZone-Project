...
cartSchema.methods.calculateTotals = async function () {
  try {
    if (
      this.items.some(
        (item) =>
          ...
          (!item.productId.offerDetails || !item.productId.regularPrice)
      )
    ) {
    ...
    if (this.couponCode) {
      ...
      if (
        coupon &&
        (!coupon.minPurchase || regularSubTotal >= coupon.minPurchase)
      ) {
    ...
    this.finalAmount = Math.max(
      0,
      regularSubTotal - productDiscount - couponDiscount
    );
...
cartSchema.pre('save', async function (next) {
  try {
    if (
      this.items.some(
        (item) =>
          typeof item.productId === 'object' && !item.productId.offerDetails
      )
    ) {
      await mongoose.model('Cart').populate(this, {
        path: 'items.productId',
