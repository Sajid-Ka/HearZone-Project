  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (attempt === maxRetries) {
        ...
        console.warn(
          `Note: File ${path.basename(filePath)} will be cleaned up by system later`
        );
        return;
      }
...
const editProduct = async (req, res) => {
  try {
    ...
    const replacedImages = data.replacedImages
      ? JSON.parse(data.replacedImages)
      : [];
    ...
    if (req.files && req.files.length > 0) {
      for (const file of req.files) {
        ...
        const replacedImageIndex = images.findIndex((img) =>
          replacedImages.includes(img)
        );
...
const deleteSingleImage = async (req, res) => {
  try {
    ...
    if (product.productImage.length <= 1) {
      return res
        .status(400)
        .json({ success: false, message: 'Cannot delete the last image' });
    }
    ...
    await Promise.all([safeDelete(imagePath), safeDelete(reImagePath)]);
